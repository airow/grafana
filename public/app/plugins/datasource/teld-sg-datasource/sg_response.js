define([
  ...
  function (_) {
    ...
    function SGResponse(targets, response) {
      this.targets = targets;
      this.response = response;
    }

    SGResponse.prototype.processMetrics = function (esAgg, target, seriesList, props) {
    ...
    SGResponse.prototype.processAggregationDocs = function (esAgg, aggDef, target, docs, props) {
    ...
    SGResponse.prototype.processBuckets = function (aggs, target, seriesList, docs, props, depth) {
    ...
    SGResponse.prototype._getSeriesName = function (series, target, metricTypeCount) {
    ...
    SGResponse.prototype.nameSeries = function (seriesList, target) {
    ...
    SGResponse.prototype.processHits = function (hits, seriesList) {
    ...
    SGResponse.prototype.trimDatapoints = function (aggregations, target) {
    ...
    SGResponse.prototype.getErrorFromSGResponse = function (response, err) {
      var result = {};
    ...
    SGResponse.prototype.getTimeSeries = function () {
      var seriesList = [];

      for (var i = 0; i < this.response.responses.length; i++) {
        var response = this.response.responses[i];
        if (response.error) {
          throw this.getErrorFromSGResponse(this.response, response.error);
        }
    ...
    SGResponse.prototype.processData = function (data, seriesList) {
      var series = { target: 'docs', type: 'docs', datapoints: [], total: data.length };
      var propName, item, doc, i;
      ...
      for (i = 0; i < data.length; i++) {
        ...
        if (item._source) {
          for (propName in item._source) {
            doc[propName] = item._source[propName];
          }
        }

        for (propName in item.fields) {
          doc[propName] = item.fields[propName];
        }
        series.datapoints.push(doc);
      }
    ...
    SGResponse.prototype.processArrayToTable = function (data, seriesList) {
      var series = {
        type: 'table',
        columns: _.map(_.keys(data[0]), function (item) { return { "text": item }; }),
        rows: _.map(data, function (item) { return _.values(item); })
      };
      seriesList.push(series);
    };

    SGResponse.prototype.getDataList = function () {
      var seriesList = [];
      ...
      for (var i = 0; i < this.response.responses.length; i++) {
        ...
        if (response.data && response.data.length > 0) {
          this.processArrayToTable(response.data, seriesList);
        }
      }
