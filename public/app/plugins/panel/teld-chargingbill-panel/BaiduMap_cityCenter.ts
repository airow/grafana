...
export function transformGeoMap() {

  let returnValue = {};

  function map(item): any {

    let match = _.split(item.g, /[,|]/);
    item.lng = match[0];
    item.lat = match[1];
    item.zoom = match[2];
    item.coord = [item.lng, item.lat];

    returnValue[item.n] = item;

    return item;
  }

  let municipalities = geoCoordMap.municipalities.map(map);

  let provinces = [];
  _.forEach(geoCoordMap.provinces, function (value, index) {
    let p = map(value);
    provinces.push(p);
    let cities = value.cities.map(map);
  });

  let other = geoCoordMap.other.map(map);
