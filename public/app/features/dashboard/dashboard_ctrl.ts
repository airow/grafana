...
export class DashboardCtrl {
    ...
    constructor(
        ...
        */
        ...
        function visibility(row, contextSrv, variableSrv) {
            var rowModel = row.model;
            if (rowModel.hideRowExpress) {
                //"vars.cTab_fffffffff==='A1'"
                var compiled = _.template('${' + rowModel.hideRowExpressString + "}", {
                    'variable': ['vars'],
                    imports: {
                        '_': _,
                        'moment': moment,
                ...
                var contextData = _.transform(variableSrv.variables, (result, variable) => {
                    result[variable.name] = variable.current.value;
                }, {});
                var panelHide = compiled(contextData) === 'true';

                // _.each(row.panels, panel => {
                //     if (contextSrv.isEditor) {
                //         panel.editorHide = !panelHide;
                //     } else {
                //         panel.dyHide = !panelHide;
                //     }
                // });
        ...
        $scope.getRowHide = function (row) {
            // debugger;
            var returnValue = true;
            if (row.model.hideRowExpress) {
                var v = !visibility(row, contextSrv, variableSrv);
                // returnValue = contextSrv.isEditor ? returnValue : v;
                returnValue = v;
                // return returnValue;
            }
            return returnValue;
